...
(function (io) {
    ...
    Socket.prototype._send = function (url, data, cb, method) {
        // Send the query.
        socket[method](url, data, function (response) {
            // Check the response.
            try{
                var message = Message.create(response);
            }catch(e){
                var message = new Message('Invalid server response', response);
            }
    ...
    Socket.prototype.send = function (url, data, cb, method) {
        var socket = this;
        ...
        // Always use the lowercase name.
        method = method.toLowerCase();

        if(method !== 'get' && !data._csrf){
            // Auto get the token.
            socket.get('/csrfToken', {}, function(res){
                data._csrf = res._csrf;
                // And send the request.
                socket._send(url, data, cb, 'get');
            });
        }else{
            // Token provided and/or get request. (The token is not required for GET requests)
            socket._send(url, data, cb, method);
        }
    };
